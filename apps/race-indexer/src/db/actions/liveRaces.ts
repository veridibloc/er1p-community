...
export async function finalizeLiveRace(raceId: string, endedAt: Date) {
  ...
  if (sortedLeaderboard.length > 0) {
    await db.insert(historicalLeaderboards).values(sortedLeaderboard);
    console.log(
  ...
  await db.delete(liveLeaderboards).where(eq(liveLeaderboards.raceId, raceId));

  // 5. Delete live race entry
  await db.delete(liveRaces).where(eq(liveRaces.raceId, raceId));
